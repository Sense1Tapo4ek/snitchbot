...
Call `snitchbot.init()` from a `lifespan` context manager so every uvicorn / hypercorn worker registers separately. Then call `install(app)` exactly once.
...
> **Why inside `lifespan`?** Each uvicorn / hypercorn worker runs the
> lifespan after the post-fork stage, so every worker registers as its
> own client in the live dashboard. Calling `init()` at module level
> works too via the post-fork hook, but the lifespan path is reliable
> on every platform (including macOS where Python's `spawn` start
> method bypasses fork hooks).
...
> Running multiple FastAPI services through one bot? Point them at a forum supergroup with `can_manage_topics` and snitchbot will give each service its own topic automatically. See [Per-service topics](./forum-mode).
